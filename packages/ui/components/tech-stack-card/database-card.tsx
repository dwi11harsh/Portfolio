import { databaseTechStackArr } from "../..";
import { CardBody, CardContainer, CardItem } from "./3d-card-elements";
import { ThreeDCard } from "./3d-cards";

export const DatabaseCard = () => {
  return (
    <CardContainer className="ml-10 mt-10 pr-20 flex flex-col bg-gray-50 relative group/card border border-black/[0.1]  rounded-xl p-6 dark:border-white/[0.2] hover:shadow-sm">
      <CardBody>
        <CardItem
        ...
        <div className="flex flex-col">
          {databaseTechStackArr.map((element) => (
            <ThreeDCard
              key={element.name}
              name={element.name}
              link={element.link}
